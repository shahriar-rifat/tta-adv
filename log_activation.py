...
def log_activation(model, layers, data_loader, device):
  
    mean_activations = {layer_id: torch.empty(0) for layer_id in layers}
    var_activations = {layer_id: torch.empty(0) for layer_id in layers}
    mean_activations_victim = {layer_id: torch.empty(0) for layer_id in layers}
    var_activations_victim = {layer_id: torch.empty(0) for layer_id in layers}
    ...
    mean_act_benign = torch.empty(0)
    var_act_benign = torch.empty(0)
    mean_act_mal= torch.empty(0)
    var_act_mal = torch.empty(0)
    ...
    for layer_id in layers:
        ...
        handles_victim[layer_id] = layer_victim.register_forward_hook(activation_hook)
    ...
    for n_batch, (inputs,labels) in enumerate(bar):
        ...
        for layer_id in layers:
            ...
            # var, mean = torch.var_mean(layer.act, dim=[2, 3], unbiased=False, keepdim=False)
            # print(f'Shape of the activation in {layer_id} = {layer.act.shape}')
            
            # var_activations[layer_id] = torch.cat((var_activations[layer_id], var), dim=0)

            layer_victim = dict([*victim_model.named_modules()])[layer_id]
            var, mean = torch.var_mean(layer_victim.act, dim=[2, 3], unbiased=False, keepdim=False)
            print(f'Shape of the activation of victim in {layer_id} = {mean.shape}')
            mean_act_benign = torch.cat((mean_act_benign, mean[:-mal_num,:]), dim=0)
            mean_act_mal = torch.cat((mean_act_mal, mean[-mal_num:,:]), dim=0)
            var_act_benign = torch.cat((var_act_benign, var[:-mal_num,:]), dim=0)
            var_act_mal = torch.cat((var_act_mal, var[-mal_num:,:]), dim=0)
            del(layer.act)
    ...
    for layer_id in layers:
        ...
        handles_victim[layer_id].remove()
    
    return mean_act_benign, mean_act_mal, var_act_benign, var_act_mal



class Activation_Hook():
    def __call__(self, module, input, output):
        module.act = output.detach().clone().cpu()
...
if __name__ == "__main__":
    ...
    activation_layers = ['layer3.2.conv2']
    ...
    for corruption in corruptions:
        ...
        mean_activations, mean_activations_victim, var_activations, var_activation_victim = log_activation(model, activation_layers, 
                                             data_loader, device)

        mean_activations, var_activations, mean_activations_victim, var_activation_victim = mean_activations.numpy(), var_activations.numpy(), mean_activations_victim.numpy(), var_activation_victim.numpy()
        np.save('mean_act.npy', mean_activations)
        np.save('var_act.npy', var_activations)
        np.save('mean_act_victim.npy',mean_activations_victim)
        np.save('var_act_victim.npy', var_activation_victim)